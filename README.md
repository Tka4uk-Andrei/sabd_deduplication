...
- [Результаты тестирования](#результаты-тестирования)
  - [Скорость работы алгоритма сжатия](#скорость-работы-алгоритма-сжатия)
  - [Скорость работы алгоритма восстановления](#скорость-работы-алгоритма-восстановления)
...
требуется реализовывать модуль по нарезке данных на одинаковые фрагменты.

### Архитектуры системы
Графическое описание архитектуры системы приведено на рисунке ниже
...
- data_compare - функция, сопоставляющая исходные байты файла с восстановленными.
...
| FOLDER_WITH_DECOMPRESED_DATA | Строка | Путь к папке с восстановленными файлами |
...
число вариантов будет превышать число, которое может вмеместить `ZIPPED_DATA_CELL_SIZE`,
...
| hash      | hash      | Хеш-сумма вычисленная для блока данных заданного размера  |
...
- 4 байт, только для `None` алгоритма сжатия

Для получения значений среднего времени работы программы каждое тестовое 
...
*Примечание.* Время работы приведено для каждого отдельного  файла.
...
В ходе работы программы во всех вариантах ошибок при сжатии/восстановлении 
не было.
...
- Скорость работы программы по сжатию файлов линейно зависимо от количества 
  обработанных файлов до этого.
- Чем больше размер блока, тем меньше время работы программы
- Время восстановления файла постоянно, скорость работы так же, как в сжатии 
...
- Добавление новых хешей происходит величины одного порядка не зависисмо от 
  количества файлов. Для меньшего размера блока большее число новых хешей
